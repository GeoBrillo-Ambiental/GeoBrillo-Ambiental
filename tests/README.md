...
php tests/test_registro_roles.php
```
...
`test_registro_roles.php` complementa a ese: verifica que el endpoint
público `registro.php` (sin token, porque todavía no existe una
sesión) rechace un intento de autoasignarse el rol Administrador
mandando `id_rol=5` directamente por POST, algo que el formulario
`registro.html` no muestra pero que cualquiera podría probar con
Postman o curl.
